from .single_stage_fsd import SingleStageFSD, SingleStageFSD_infer
import torch
...
@DETECTORS.register_module()
class FSD_infer(SingleStageFSD_infer):
    ...
    def prepare_multi_class_roi_input(self, points, cluster_pts_feats, pts_seg_feats, pts_mask, pts_batch_inds,
                                      cluster_pts_xyz):
        assert isinstance(pts_mask, list)
        bg_mask = sum(pts_mask) == 0
        assert points.shape[0] == pts_seg_feats.shape[0] == bg_mask.shape[0] == pts_batch_inds.shape[0]
    ...
    def before_second_stage(self, points_xyzs, points_feat, coords, centers, img_metas):
        rpn_outs = super().simple_test(points_xyzs, points_feat, coords, centers,
                                       img_metas=img_metas)
        proposal_list = rpn_outs['proposal_list']
        ...
        if self.num_classes > 1 or self.test_cfg.get('enable_multi_class_test', False):
            ...
            prepare_func = self.prepare_roi_input
        points = torch.cat([rpn_outs['all_input_points_xyzs'], rpn_outs['all_input_points_feat']], dim=-1)
        val_points = torch.cat([rpn_outs['valid_points_xyzs'], rpn_outs['valid_points_feat']], dim=-1)
        pts_xyz, pts_feats, pts_batch_inds = prepare_func(
            points,
            rpn_outs['valid_pts_feats'],
            rpn_outs['seg_feats'],
        ...
        # points_xyzs = pts_xyz[:, :3]
        # points_feat = pts_xyz[:, 3:]
        ...
        ext_pts_inds, ext_pts_roi_inds, ext_pts_info = self.roi_head.roi_extractor(
        ...
        new_points_xyzs = pts_xyz[ext_pts_inds][:, :3]
        new_points_feat = pts_xyz[ext_pts_inds][:, 3:]
        return new_points_xyzs, new_points_feat, new_pts_feats, ext_pts_info, ext_pts_roi_inds, rois, labels_3d, cls_preds
    def simple_test(self, points_xyzs, points_feat, coords, centers, img_metas, imgs=None, rescale=False, gt_bboxes_3d=None, gt_labels_3d=None):

        rpn_outs = super().simple_test(
            points_xyzs, points_feat, coords, centers,
            img_metas=img_metas,
            gt_bboxes_3d=gt_bboxes_3d,
            gt_labels_3d=gt_labels_3d,
        )
