...
@SEGMENTORS.register_module()
@DETECTORS.register_module()
class VoteSegmentor_infer(Base3DSegmentor):
    ...
    def __init__(self,
        ...
        self.voxel_layer = Voxelization(**voxel_layer)
        assert voxel_encoder['type'] == 'DynamicScatterVFE_infer'
        self.voxel_encoder = builder.build_voxel_encoder(voxel_encoder)
        self.middle_encoder = builder.build_middle_encoder(middle_encoder)
        self.backbone = build_backbone(backbone)
        self.segmentation_head = build_head(segmentation_head)
    ...
    def extract_feat(self, points_xyzs, points_feats, in_coors, centers):
        """Extract features from points."""
        # batch_points, coors = self.voxelize(points)
        batch_points = points_xyzs
        coors = in_coors
        # coors = coors.long()
        voxel_features, voxel_coors, voxel2point_inds, count = self.voxel_encoder(points_xyzs, points_feats, coors,
                                                                                  centers, return_inv=True)
        voxel_info = self.middle_encoder(voxel_features, voxel_coors)
        x = self.backbone(voxel_info)[0]
        padding = -1
        voxel_coors_dropped = x['voxel_feats']  # bug, leave it for feature modification
        if 'shuffle_inds' not in voxel_info:
            ...
            # this branch only used in SST-based FSD
            voxel_feats_reorder = self.reorder(x['voxel_feats'], voxel_info['shuffle_inds'],
                                               voxel_info['voxel_keep_inds'],
                                               padding)  # 'not consistent with voxel_coors any more'

        out = self.decode_neck(batch_points, coors, centers, voxel_feats_reorder, voxel2point_inds, padding)
    ...
    def forward_train(self,
        ...
        if as_subsegmentor:
            loss_decode, preds_dict = self.segmentation_head.forward_train(feats, img_metas, labels, vote_targets,
                                                                           vote_mask, return_preds=True)
            losses.update(loss_decode)
            ...
            loss_decode = self.segmentation_head.forward_train(feats, img_metas, labels, vote_targets, vote_mask,
                                                               return_preds=False)
            losses.update(loss_decode)
            output_dict = losses
    ...
    def simple_test(self, points_xyzs, points_feats, coors, centers, img_metas, gt_bboxes_3d=None, gt_labels_3d=None,
                    rescale=False):
        ...
        if self.use_multiscale_features:
            x, pts_coors, points, decoder_features = self.extract_feat(points_xyzs, points_feats, coors, centers)
        else:
            x, pts_coors, points = self.extract_feat(points_xyzs, points_feats, coors, centers)
            decoder_features = None
        ...
        valid_pts_mask = x[1]
        points = points[valid_pts_mask]
        points_feats = points_feats[valid_pts_mask]
        pts_coors = pts_coors[valid_pts_mask]
        ...
        output_dict = dict(
            seg_points_xyzs=points,
            seg_points_feat=points_feats,
            seg_logits=seg_logits,
            seg_vote_preds=vote_preds,
            offsets=offsets,
            seg_feats=feats,
...
@DETECTORS.register_module()
class SingleStageFSD_infer(SingleStage3DDetector):
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(SingleStageFSD_infer, self).__init__(
            backbone=backbone,
            neck=neck,
            bbox_head=bbox_head,
            train_cfg=train_cfg,
        ...
        if 'radius' in cluster_assigner:
            ...
            self.cluster_assigner = ClusterAssigner_infer(**cluster_assigner)
        self.cluster_assigner.num_classes = self.num_classes
        self.print_info = {}
        self.as_rpn = bbox_head.get('as_rpn', False)
    ...
    def extract_feat(self, points_xyzs, points_feat, pts_feats, pts_cluster_inds, img_metas, center_preds):
        """Extract features from points."""
        new_coors, unq_inv = torch.unique(pts_cluster_inds, return_inverse=True, return_counts=False, dim=0)
    ...
    def forward_train(self,
        ...
        seg_out_dict = self.segmentor(points=points, img_metas=img_metas, gt_bboxes_3d=gt_bboxes_3d,
                                      gt_labels_3d=gt_labels_3d, as_subsegmentor=True)
        ...
        if self.cfg.get('pre_voxelization_size', None) is not None:
            dict_to_sample = self.pre_voxelize(dict_to_sample)
        sampled_out = self.sample(dict_to_sample, dict_to_sample['vote_offsets'], gt_bboxes_3d,
                                  gt_labels_3d)  # per cls list in sampled_out

        # we filter almost empty voxel in clustering, so here is a valid_mask
        cluster_inds_list, valid_mask_list = self.cluster_assigner(sampled_out['center_preds'],
                                                                   sampled_out['batch_idx'], gt_bboxes_3d, gt_labels_3d,
                                                                   origin_points=sampled_out[
                                                                       'seg_points'])  # per cls list
        pts_cluster_inds = torch.cat(cluster_inds_list, dim=0)  # [N, 3], (cls_id, batch_idx, cluster_id)

        num_clusters = len(torch.unique(pts_cluster_inds, dim=0)) * torch.ones((1,),
                                                                               device=pts_cluster_inds.device).float()
        losses['num_clusters'] = num_clusters
        ...
        outs = self.bbox_head(cluster_feats, cluster_xyz, cluster_inds)
        loss_inputs = (outs['cls_logits'], outs['reg_preds']) + (cluster_xyz, cluster_inds) + (
    ...
    def pre_voxelize(self, data_dict):
        batch_idx = data_dict['batch_idx']
        points = data_dict['seg_points_xyzs']
        ...
        for data_name in data_dict:
            data = data_dict[data_name]
            if data.dtype in (torch.float, torch.float16):
                voxelized_data, voxel_coors = scatter_v2(data, coors, mode='avg', return_inv=False, new_coors=new_coors,
                                                         unq_inv=unq_inv)
                voxelized_data_dict[data_name] = voxelized_data
    ...
    def simple_test(self, points_xyzs, points_feats, coords, centers, img_metas, imgs=None, rescale=False, gt_bboxes_3d=None, gt_labels_3d=None):
        """Test function without augmentaiton."""
        # if gt_bboxes_3d is not None:
        #     gt_bboxes_3d = gt_bboxes_3d[0]
        #     gt_labels_3d = gt_labels_3d[0]
        ...
        #     gt_bboxes_3d = [b[l >= 0] for b, l in zip(gt_bboxes_3d, gt_labels_3d)]
        #     gt_labels_3d = [l[l >= 0] for l in gt_labels_3d]
        seg_out_dict = self.segmentor.simple_test(points_xyzs, points_feats, coords, centers, img_metas)
        seg_feats = seg_out_dict['seg_feats']

        dict_to_sample = dict(
            seg_points_xyzs=seg_out_dict['seg_points_xyzs'],
            seg_points_feat=seg_out_dict['seg_points_feat'],
            seg_logits=seg_out_dict['seg_logits'],
            seg_vote_preds=seg_out_dict['seg_vote_preds'],
            seg_feats=seg_feats,
            batch_idx=seg_out_dict['batch_idx'],
        ...
        pts_feats = torch.cat([combined_out['seg_logits'], combined_out['seg_vote_preds'], combined_out['seg_feats']],
                              dim=1)
        assert len(pts_cluster_inds) == len(points_xyzs) == len(pts_feats)

        extracted_outs = self.extract_feat(points_xyzs, points_feat, pts_feats, pts_cluster_inds, img_metas, combined_out['center_preds'])
        cluster_feats = extracted_outs['cluster_feats']
        cluster_xyz = extracted_outs['cluster_xyz']
        cluster_inds = extracted_outs['cluster_inds']
        assert (cluster_inds[:, 1] == 0).all()
        ...
        if self.as_rpn:
            output_dict = dict(
                all_input_points_xyzs=dict_to_sample['seg_points_xyzs'],
                all_input_points_feat=dict_to_sample['seg_points_feat'],
                valid_pts_feats=extracted_outs['cluster_pts_feats'],
                valid_points_xyzs=extracted_outs['cluster_points_xyzs'],
                valid_points_feat=extracted_outs['cluster_points_feat'],
                seg_feats=dict_to_sample['seg_feats'],
                pts_mask=sampled_out['fg_mask_list'],
                pts_batch_inds=dict_to_sample['batch_idx'],
                proposal_list=bbox_list
    ...
    def sample(self, dict_to_sample, offset, gt_bboxes_3d=None, gt_labels_3d=None):
        ...
        for cls in range(self.num_classes):
            ...
            fg_mask = self.get_fg_mask(seg_scores, seg_points_xyzs, cls, batch_idx, gt_bboxes_3d, gt_labels_3d)
            ...
            this_offset = offset[fg_mask, cls, :]
            this_points = seg_points_xyzs[fg_mask, :]
            this_centers = this_points + this_offset
            center_preds_list.append(this_centers)
    ...
    def get_fg_mask(self, seg_scores, seg_points_xyzs, cls_id, batch_inds, gt_bboxes_3d, gt_labels_3d):
        if self.training and self.train_cfg.get('disable_pretrain', False) and not self.runtime_info.get(
        ...
        if cfg.get('add_gt_fg_points', False):
            bsz = len(gt_bboxes_3d)
            assert len(seg_scores) == len(seg_points_xyzs) == len(batch_inds)
            point_list = self.split_by_batch(seg_points_xyzs, batch_inds, bsz)
            gt_fg_mask_list = []
...
class ClusterAssigner_infer(torch.nn.Module):
    ''' Generating cluster centers for each class and assign each point to cluster centers
    '''
