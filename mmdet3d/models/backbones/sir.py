...
class SIR_infer(nn.Module):
    ...
    def forward(self, points_xyzs, points_feat, features, coors, f_cluster=None):

        if self.unique_once:
            new_coors, unq_inv, count = torch.unique(coors, return_inverse=True, return_counts=True, dim=0)
        else:
            new_coors = unq_inv = None
        ...
        cluster_feat_list = []
        for i, block in enumerate(self.block_list):
            # in_feats = [points_xyzs, out_feats]
            if i < self.num_blocks - 1:
                out_feats, out_cluster_feats = block(points_xyzs, points_feat, out_feats, coors, f_cluster, unq_inv_once=unq_inv,
                                                     new_coors_once=new_coors, count=count)
                cluster_feat_list.append(out_cluster_feats)
            if i == self.num_blocks - 1:
                out_feats, out_cluster_feats, out_coors = block(points_xyzs, points_feat, out_feats, coors, f_cluster, return_both=True,
                                                                unq_inv_once=unq_inv, new_coors_once=new_coors, count=count)
                cluster_feat_list.append(out_cluster_feats)
