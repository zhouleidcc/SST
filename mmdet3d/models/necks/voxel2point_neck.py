...
@NECKS.register_module()
class Voxel2PointScatterNeck_infer(nn.Module):
    """
    A memory-efficient voxel2point with torch_scatter
    """
    ...
    def forward(self, points, pts_coors, pts_centers, voxel_feats, voxel2point_inds, voxel_padding=-1):
        """Forward function.
        ...
        pts_mask = torch.all((pts_feats != voxel_padding), dim=1)
        if self.with_xyz:
            pts_feats = pts_feats[pts_mask]
            pts_coors = pts_coors[pts_mask]
            points = points[pts_mask]
            ...
            # voxel_size = torch.tensor(self.voxel_size, dtype=dtype, device=device).reshape(1, 3)
            # pc_min_range = torch.tensor(self.point_cloud_range[:3], dtype=dtype, device=device).reshape(1, 3)
            # voxel_center_each_pts = (pts_coors[:, [3, 2, 1]].to(dtype).to(
            #     device) + 0.5) * voxel_size + pc_min_range  # x y z order
            # local_xyz = points[:, :3] - voxel_center_each_pts
            # result0 = local_xyz
            # result1 = pts_centers
            # cos_similarity = torch.sum(result1 * result0) / \
            #                  torch.sqrt(torch.sum(result1 * result1) * torch.sum(result0 * result0))
            # assert (cos_similarity == 1.0).all()
            if self.normalize_local_xyz:
                local_xyz = local_xyz / (voxel_size / 2)

            if self.training and not self.normalize_local_xyz:
                assert (
                            local_xyz.abs() < voxel_size / 2 + 1e-3).all(), 'Holds in training. However, in test, this is not always True because of lack of point range clip'
            results = torch.cat([pts_feats, local_xyz], 1)
        else:
            results = pts_feats[pts_mask]
