...
@HEADS.register_module()
class FullySparseBboxHead_infer(BaseModule):
    ...
    def __init__(self,
        ...
        if reg_mlp is not None:
            self.conv_reg = build_mlp(end_channel, reg_mlp + [self.box_code_size, ], norm_cfg, True, act=act,
                                      dropout=reg_dropout)
        else:
            self.conv_reg = nn.Linear(end_channel, self.box_code_size)
    ...
    @force_fp32(apply_to=('pts_features', 'rois'))
    def forward(self, points_xyzs, points_feat, pts_features, pts_info, roi_inds, rois):
        """Forward pass.
        ...
        rois = rois[:, 1:]
        roi_centers = rois[:, :3]
        rel_xyz = points_xyzs - roi_centers[roi_inds]

        if self.unique_once:
            new_coors, unq_inv, count = torch.unique(roi_inds, return_inverse=True, return_counts=True, dim=0)
        else:
            new_coors = unq_inv = None

        out_feats = pts_features
        f_cluster = torch.cat(
            [pts_info['local_xyz'], pts_info['boundary_offset'], pts_info['is_in_margin'][:, None], rel_xyz], dim=-1)

        cluster_feat_list = []
        for i, block in enumerate(self.block_list):

            # in_feats = torch.cat([points_xyzs, points_feat, out_feats], 1)
            features = out_feats

            if self.geo_input:
                features = torch.cat([features, f_cluster * 0.1], 1)

            if i < self.num_blocks - 1:
                # return point features
                out_feats, out_cluster_feats = block(points_xyzs, points_feat, features, roi_inds, f_cluster, unq_inv_once=unq_inv,
                                                     new_coors_once=new_coors, count=count)
                cluster_feat_list.append(out_cluster_feats)
            if i == self.num_blocks - 1:
                # return group features
                out_cluster_feats, out_coors = block(points_xyzs, points_feat, features, roi_inds, f_cluster, unq_inv_once=unq_inv,
                                                     new_coors_once=new_coors, count=count)
                cluster_feat_list.append(out_cluster_feats)
    ...
    def loss(self, cls_score, bbox_pred, nonempty_roi_mask, rois, labels, bbox_targets, pos_batch_idx,
        ...
        if pos_inds.any() == 0:
            ...
            bbox_targets = self.filter_pos_assigned_but_empty_rois(bbox_targets, pos_batch_idx, pos_inds,
                                                                   rois[:, 0].int())
            ...
            if self.with_corner_loss:
                ...
                # calculate corner loss
                assert pos_gt_bboxes.size(0) == pos_gt_labels.size(0)
                pos_gt_bboxes = self.filter_pos_assigned_but_empty_rois(pos_gt_bboxes, pos_batch_idx, pos_inds,
                                                                        rois[:, 0].int())
                pos_gt_labels = self.filter_pos_assigned_but_empty_rois(pos_gt_labels, pos_batch_idx, pos_inds,
                                                                        rois[:, 0].int())
                if self.train_cfg.get('corner_loss_only_car', True):
                    car_type_index = self.train_cfg['class_names'].index('Car')
                    car_mask = pos_gt_labels == car_type_index
    ...
    def get_targets(self, sampling_results, rcnn_train_cfg, concat=True):
        ...
        if concat:
            ...
            bbox_target_batch_idx = torch.cat(
                [t.new_ones(len(t), dtype=torch.int) * i for i, t in enumerate(bbox_targets)])
            bbox_targets = torch.cat(bbox_targets, 0)
            pos_gt_bboxes = torch.cat(pos_gt_bboxes, 0)
            pos_gt_labels = torch.cat(pos_gt_labels, 0)
            reg_mask = torch.cat(reg_mask, 0)
    ...
    def get_bboxes(
        ...
        """
        assert rois.size(0) == cls_score.size(0) == bbox_pred.size(0)
        assert isinstance(class_labels, list) and isinstance(class_pred, list) and len(class_labels) == len(
            class_pred) == 1
