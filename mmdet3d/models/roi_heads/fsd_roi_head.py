...
@HEADS.register_module()
class GroupCorrectionHead_infer(Base3DRoIHead):
    """Part aggregation roi head for PartA2.
    ...
    def simple_test(
        ...
        assert len(
            proposal_list) == 1, 'only support bsz==1 to make cls_preds and labels_3d consistent with bbox_results'
        rois = bbox3d2roi([res[0].tensor for res in proposal_list])
        cls_preds = [res[1] for res in proposal_list]
        labels_3d = [res[2] for res in proposal_list]
        ...
        bbox_results = self._bbox_forward(points_xyzs, points_feat, pts_feats, pts_batch_inds, rois)
    ...
    def _bbox_forward(self, points_xyzs, points_feat, pts_feats, batch_idx, rois):

        assert points_xyzs.size(0) == pts_feats.size(0) == batch_idx.size(0)

        ext_pts_inds, ext_pts_roi_inds, ext_pts_info = self.roi_extractor(
            points_xyzs,  # intensity might be in points_xyzs
            batch_idx,
            rois[:, :8],
            batch_size=1 if not self.training else None,
        )
        ...
        new_pts_feats = pts_feats[ext_pts_inds]
        new_points_xyzs = points_xyzs[ext_pts_inds]
        new_points_feat = points_feat[ext_pts_inds]

        # def forward(self, points_xyzs, pts_features, pts_info, roi_inds, rois):
    ...
    def _assign_and_sample(self, proposal_list, gt_bboxes_3d, gt_labels_3d):
        ...
        for batch_idx in range(len(proposal_list)):
            ...
            if no_proposal:
                # print('*******fake a box*******')
                if cur_boxes.tensor.size(1) == 7:
                    cur_boxes = LiDARInstance3DBoxes(
                        torch.tensor([[0, 0, 5, 1, 1, 1, 0]], dtype=torch.float32, device=cur_boxes.device))
                else:
                    cur_boxes = LiDARInstance3DBoxes(
                        torch.tensor([[0, 0, 5, 1, 1, 1, 0, 0, 0]], dtype=torch.float32, device=cur_boxes.device),
                        box_dim=9)
                cur_scores = torch.tensor([0.0], dtype=torch.float32, device=cur_boxes.device)
                cur_pd_labels = torch.tensor([0], dtype=torch.int64, device=cur_boxes.device)
