...
class segment_decode(nn.Module):
    ...
    def get_fg_mask_topK(self, seg_scores, cls_id):
        ks = [9933, 249, 12]
        seg_scores = seg_scores[:, cls_id]
        topk_score, topk_indices = torch.topk(seg_scores, ks[cls_id], sorted=False)
    ...
    def get_fg_mask(self, seg_scores, cls_id):
        ...
        topk_score, topk_indices = torch.topk(seg_scores, ks[cls_id], sorted=False)
        return fg_mask, topk_score, topk_indices
    ...
    def clusster_assign(self, points, batch_idx, class_name, i):
        ...
        """
        tmp_num_points = unq_cnt.cpu().numpy().shape[0]
        tmp_num_points = torch.Tensor([tmp_num_points]).to(device=unq_cnt.device, dtype=unq_cnt.dtype)
        tmp_sampled_centers = self.scatter_infer(points, unq_inv, unq_cnt, tmp_num_points, 'mean')
        tmp_voxel_coors = new_coors
        tmp_dist = self.connected_dist[class_name]
        tmp_cluster_inds = self.collect_component(tmp_sampled_centers, tmp_voxel_coors[:, 0], tmp_dist).unsqueeze(dim=1)
        tmp_cluster_inds_per_point = tmp_cluster_inds[unq_inv]
        print('ok')
        """
    ...
    def get_fg_feats(self, feats, fg_indice, cc_indice, offset):
        assert len(fg_indice) == len(cc_indice) == self.num_classes
        cc = [cc_indice[i] + offset[0][i] for i in range(self.num_classes)]
        fg = [fg_indice[i] + offset[1][i] for i in range(self.num_classes)]

        cc_indices = torch.cat(cc)
        fg_indices = torch.cat(fg)
        tmp = fg_indices.max()
        fg_indices = fg_indices[cc_indices]
        feats = feats[fg_indices]
        return feats
    ...
    def forward(self, points_xyzs, points_feat, pts_coors, pts_centers, voxel_feats, voxel2point_inds, decoder_coors, batch_idx):
        ...
        all_datas = torch.cat([points_xyzs, points_feat, seg_logits, vote_preds, feats, offsets], dim=1)
        all_datas = self.scatter_infer(all_datas, decoder_unq_inv, count, num_points, 'mean')

        fg_indice = []
        pred_center_list = []
        new_cluster_inds_list, new_valid_mask_list = [], []
        for cls in range(self.num_classes):
            top_k_score, top_k_indice = self.get_fg_mask_topK(all_datas[:, 5:8], cls)
            fg_indice.append(top_k_indice)
            start = -9 + 3 * cls
            end = -9 + 3 * (cls + 1)
            pre_offsets = all_datas[:, start:end] if end != 0 else all_datas[:, start:]
            center_pre = all_datas[:, :3] + pre_offsets
            points = center_pre[top_k_indice]
            pred_center_list.append(center_pre[top_k_indice])
            b_idx = batch_idx[top_k_indice]
            class_name = self.class_names[cls]
            cluster_inds, valid_mask = self.clusster_assign(points, b_idx, class_name, cls)
            new_cluster_inds_list.append(cluster_inds)
            new_valid_mask_list.append(valid_mask[0])
        new_pts_cluster_inds = torch.cat(new_cluster_inds_list, dim=0)
        pred_centers = torch.cat(pred_center_list, dim=0)
        indice_offset = [0, 9933, 9933 + 249]
        # new_feats = self.get_fg_feats(feats, fg_lists, valid_lists, indice_offset)

        tmp = torch.cat([a + b for a, b in zip(new_valid_mask_list, indice_offset)])
        tmp_fg_indice = torch.cat(fg_indice)[tmp]
        new_center_pred = pred_centers[tmp]
        new_feats = all_datas[tmp_fg_indice]
        extracted_outs = self.extract_feat(new_feats[:, 0:3], new_feats[:, 3:5], new_feats[:, 5:84],
                                           new_pts_cluster_inds, new_center_pred)
        check = False
        if check:
            ...
            center_preds_list = []
            for cls in range(self.num_classes):
                fg_mask, top_k_score, top_k_indice = self.get_fg_mask(seg_scores, cls)
                cls_offset = offset[:, cls, :]
                bool_indice = self.scatter_bool(fg_mask)
                fg_mask_list.append([fg_mask, top_k_indice])
                this_offset = cls_offset[top_k_indice]
                this_points = seg_points_xyzs[top_k_indice]
            ...
            center_preds = torch.cat(output_dict['center_preds'], dim=0)
            ...
            seg_points_xyzs = dict_to_sample['seg_points_xyzs']
            seg_points_feat = dict_to_sample['seg_points_feat']
            seg_logits = dict_to_sample['seg_logits']
            seg_vote_preds = dict_to_sample['seg_vote_preds']
            seg_feats = dict_to_sample['seg_feats']
            vote_offsets = dict_to_sample['vote_offsets']
            feats = torch.cat([seg_points_xyzs, seg_points_feat, seg_logits, seg_vote_preds, \
                               seg_feats, vote_offsets], dim=1)

            cos_feat = cos_similarity(feats, all_datas)

            tmpoffset = torch.cat([offset[:, cls, :] for cls in range(self.num_classes)], dim=0)
            fg_lists = [a[1] for a in fg_mask_list]
            valid_lists = [a[0] for a in valid_mask_list]
            indice_offset = [[0, 9933, 9933 + 249], [0, 0, 0]]
            new_feats0 = self.get_fg_feats(feats, fg_lists, valid_lists, indice_offset)
            tmp = [a + b for a, b in zip(valid_lists, indice_offset[0])]
            new_center_pred = pred_centers[torch.cat(tmp)]
            cos_center = cos_similarity(new_center_pred, combined_out['center_preds'])

            indice_offset[1][1] = offset.cpu().numpy().shape[0]
            indice_offset[1][2] = offset.cpu().numpy().shape[0] * 2
            new_offsets = self.get_fg_feats(tmpoffset, fg_lists, valid_lists, indice_offset)
            new_centers = new_feats[:, :3] + new_offsets

            cos_sim = cos_similarity(new_centers, combined_out['center_preds'])
            cos_points_xyzs = cos_similarity(new_feats[:, 0:3], points_xyzs)
            cos_points_feat = cos_similarity(new_feats[:, 3:5], points_feat)
            cos_pts_feats = cos_similarity(new_feats[:, 5:84], pts_feats)
            ...
            extracted_outs0 = self.extract_feat(points_xyzs, points_feat, pts_feats, pts_cluster_inds, combined_out['center_preds'])
            indice_sim = cos_similarity(new_pts_cluster_inds, pts_cluster_inds)
            cos_sim = {}
            for k, v in extracted_outs.items():
                cos_sim[k] = cos_similarity(extracted_outs0[k], v)
...
def main():
    ...
    infer_points, coords, point_centers_off = infer_voxelize(infer_points, voxel_layer, encoder)
    batch_idx = coords[:, 0]
    voxel_coords = pre_voxelize(model, infer_points, batch_idx)
    args.export = True
    ...
    with torch.no_grad():
        if args.export == False:
            out = model.simple_test(infer_points_xyzs, infer_points_feat, coords, point_centers_off, imetas)[0]
            print(out)
            obstacles = {
            ...
            out_stage_two_pre_process = stage_two_pre_process(infer_points_xyzs, infer_points_feat, coords, point_centers_off, imetas)
            test = stage_two_process(*out_stage_two_pre_process)
            # export_out = encoder_export(infer_points_xyzs, infer_points_feat, coords, point_centers_off)
            ...
            # last = deepcopy(voxel_coords[:, 0:1].int())
            # segment_out = segment_decoder(infer_points_xyzs, infer_points_feat, coords, point_centers_off, result1, export_out[2], voxel_coords, last)
            #
            # print("cos similarity %f of between segement modified model and origin model" % cos_simi)
            torch.onnx.export(encoder_export,
                (infer_points_xyzs, infer_points_feat, coords, point_centers_off),
                "voxel_encoder.onnx",
                opset_version=11,
                input_names=["points_xyzs", "points_feat", "coords", "point_centers_off"],
                output_names=["voxel_feat", "voxel_coords", "voxel2point_inds"],
                # example_outputs=export_out
                              )

            # torch.onnx.export(segment_decoder,
            #                   (infer_points_xyzs, infer_points_feat, coords, point_centers_off, result1, export_out[2], voxel_coords, last),
            #                   "stage_one.onnx",
            #                   opset_version=17,
            #                   input_names=["points_feat", "points_feat", "coords", "point_centers_off", "voxel_feats", "voxel2point_inds", "voxel_coords"],
            #                   # output_names=["voxel_feat", "voxel_coords", "voxel2point_inds"],
            ...
            torch.onnx.export(stage_two_process,
                              ...
                              "stage_two.onnx",
                              opset_version=17,
                              # input_names=["points", "coords", "point_centers_off", "voxel_feats", "voxel2point_inds",
                              #              "voxel_coords"],
                              # output_names=["voxel_feat", "voxel_coords", "voxel2point_inds"],
